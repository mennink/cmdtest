...
echo "symbol = \"➜\"     # The \"symbol\" segment is being set to \"➜\"" >> ~/.config/starship.toml

if [ "${USERNAME}" != "root" ]; then
    mkdir -p /home/$USERNAME/.config    
    cp -f /root/.config/starship.toml /home/$USERNAME/.config        
...
if [ "${DOT_LOCAL_ALREADY_ADDED}" != "true" ]; then
    echo "eval \"\$(starship init bash)\"" | tee -a /root/.bashrc >> /home/$USERNAME/.bashrc
...
if [ "${INSTALL_ZSH}" = "true" ] && [ "${ZSH_ALREADY_INSTALLED}" != "true" ]; then
    apt-get-update-if-needed
